...
def pm_forgot_password(request):
    if request.method == 'POST':
        email = request.POST.get('email')
        try:
            pm = postman.objects.get(email=email)
            otp = random.randint(100000, 999999)
            pm.otp = otp
            pm.save()
            subject = "Password Reset OTP"
...
def pm_verify_otp(request):
    if request.method == 'POST':
        email = request.session.get('reset_email')
        otp_entered = request.POST.get('otp')
        otp_timestamp = request.session.get('otp_timestamp', 0)
        if time.time() - otp_timestamp > 30:
            return HttpResponse("<script>alert('OTP expired! Request a new one.'); window.location.href='/pm_forgot_password/';</script>")
        try:
            user = postman.objects.get(email=email)
            if str(user.otp) == otp_entered:
                return redirect('pm_reset_password')
            else:
                return HttpResponse("<script>alert('Invalid OTP'); window.location.href='/pm_verify_otp/';</script>")
        except postman.DoesNotExist:
            return redirect('pm_forgot_password')
    return render(request, 'pm_verify_otp.html')
def pm_reset_password(request):
    if request.method == 'POST':
        email = request.session.get('reset_email')
        new_password = request.POST.get('new_password')
        try:
            pm = postman.objects.get(email=email)
            pm.password = make_password(new_password)
            pm.otp = None
            pm.save()
            request.session['reset_email'] = email
            request.session['otp_timestamp'] = time.time()
            return HttpResponse("<script>alert('Password reset successful! Please log in with your new password.'); window.location.href='/postmanlogin/';</script>")
        except postman.DoesNotExist:
            return redirect('pm_forgot_password')
    return render(request, 'pm_reset_password.html')